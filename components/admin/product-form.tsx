...
import { useAdminCategories } from "@/lib/admin/hooks/use-admin-categories"
...
export default function ProductForm({ initialData, productId, mode }: ProductFormProps) {
    ...
    // Default category selection if empty
    useEffect(() => {
        if (!initialData && formData.category === "" && categories.length > 0) {
            setFormData(prev => ({ ...prev, category: categories[0].slug }))
        }
    }, [initialData, formData.category, categories])

...
export default function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  useEffect(() => {
    try {
      const storedFavorites = localStorage.getItem('madu-favorites')
      if (storedFavorites) {
  ...
  const toggleFavorite = (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (newState) {
        ...
        favoritesArray = favoritesArray.filter((id: number) => id !== product.id)
      }
      
      localStorage.setItem('madu-favorites', JSON.stringify(favoritesArray))
    } catch (e) {
      console.error("Could not save favorite to localStorage", e)
    }
  }
